...
class TwoStepTaskExpForLLM(Experiment):
    ...
    def add_arguments_(self):
        # Add any additional arguments hereRandomSerialMemoryLLM
        self.parser.add_argument(
            '--arms', nargs='+', default=['X', 'Y'], help='List of engines to use')
        ...
        self.parser.add_argument(
            '--num_runs', type=int, default=100, help='Number of runs')
    ...
    def run_single_experiment(self, llm):
        ...
        def llm_choice(x, arms): return self.keep_arms(llm.generate(x), arms)
        ...
        if self.parser.parse_args().version_number == '1':
            for i in range(self.num_trials):
                ...
                for count, interaction in enumerate(previous_interactions):
                    days = " day" if (
                        len(previous_interactions) - count) == 1 else " days"
                    total_text += "- " + \
                        str(len(previous_interactions) - count) + days + " ago, "
                ...
                action2 = llm_choice(total_text, arms=(
                    'D', 'F') if state == 'X' else ('J', 'K'))
                treasure = np.random.binomial(
                    1, reward_probs[action_to_index[action2]], 1)[0]

                row = [i, action1, state, action2, treasure, reward_probs[0],
                       reward_probs[1], reward_probs[2], reward_probs[3]]
                data.append(row)
                ...
                if treasure:
                    feedback_item = "you boarded the spaceship to planet " + action1 + ", arrived at planet " + \
                        state + ", traded with alien " + action2 + ", and received treasures.\n"
                else:
                    feedback_item = "you boarded the spaceship to planet " + action1 + ", arrived at planet " + \
                        state + ", traded with alien " + action2 + ", and received junk.\n"
        ...
        if self.parser.parse_args().version_number == '2':
            for i in range(self.num_trials):
                ...
                if treasure:
                    feedback_item = "you told your magical carpet to fly to mountain " + action1 + \
                        ", arrived at mountain " + state + ", performed for genie " + \
                        action2 + ", and received one gold coin.\n"
                else:
                    feedback_item = "you told your magical carpet to fly to mountain " + action1 + \
                        ", arrived at mountain " + state + ", performed for genie " + \
                        action2 + ", and received nothing.\n"
                previous_interactions.append(feedback_item)

        df = pd.DataFrame(data, columns=[
                          'trial', 'action1', 'state', 'action2', 'reward', 'probsA', 'probsB', 'probsC', 'probsD'])
        return df
