...
if __name__ == '__main__':
    ...
    base_agents = ['gpt-4', 'text-bison@002',
                   'claude-2', 'claude-1', 'llama-2-70']
    including_agents = [agent + '_cot' for agent in base_agents] + \
        [agent + '_sb' for agent in base_agents] + base_agents
    ...
    for idx, row in enumerate(df.iterrows()):
        ...
        try:
            ...
            df.loc[idx, increasing_param_feature] = llm_df.loc[llm_df['Engine']
                                                               == engine, increasing_param_feature].values[0]
            df.loc[idx, 'Use of RLHF'] = llm_df.loc[llm_df['Engine']
                                                    == engine, 'Use of RLHF'].values[0]
            df.loc[idx, 'Open Source'] = llm_df.loc[llm_df['Engine']
                                                    == engine, 'Open Source'].values[0]
            df.loc[idx, 'Chain of Thought'] = llm_df.loc[llm_df['Engine']
                                                         == engine, 'Chain of Thought'].values[0]
            df.loc[idx, 'Step Back'] = llm_df.loc[llm_df['Engine']
                                                  == engine, 'Step Back'].values[0]
            df.loc[idx, 'Finetuned version of LLM'] = llm_df.loc[llm_df['Engine']
                                                                 == engine, 'Finetuned version of LLM'].values[0]
        except:
    ...
    llm_groups = []
    llm_group_names = []
    df['No of Parameters'] = pd.to_numeric(
        df['No of Parameters'], errors='coerce')
    listofsorted_engines = df.groupby('engine')['No of Parameters'].mean(
    ...
    for score_name in score_names:
        ...
        comparison_barplot(df, experiment, score_name,
                           llm_groups, llm_group_names)
