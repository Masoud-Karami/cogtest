...
def aggregated_results(dfs, score_name, experiment):
    ...
    for i, df in enumerate(dfs):
        ...
        for model in df[df[type_of_cdt] == 0].engine.unique().tolist():
            ...
            dfcdt = df[df['engine'] == model +
                       '_cot'] if i == 0 else df[df['engine'] == model + '_sb']
        ...
        cis_diffs.append(ci)
        print(
            f'Aggregated mean difference for {type_of_cdt}: {weighted_mean_diff} +- {ci}')
...
def regression(df, experiment, score, features, score_name='n/a', regtype='behavior'):
    ...
    for row in df.iterrows():
        ...
        if df['Finetuned version of LLM'][row[0]] not in typesofengine:
            df_standardized['type_of_engine'][row[0]
                                              ] = df['Finetuned version of LLM'][row[0]]
    ...
    md = smf.mixedlm(f"{score} ~ { ' + '.join(features) }",
                     df_standardized, groups=df_standardized['type_of_engine'])
    mdf = md.fit(maxiter=10000)

    if not mdf.converged:
        print(
            f'Experiment {experiment} and score {score_name} did not converge')
    ...
    coefficients.index = [feature.replace(
        '_', ' ').title() for feature in coefficients.index]

    # Create a dataframe with feature names and their corresponding coefficients
    coeff_df = pd.DataFrame(
        {'Feature': coefficients.index, 'Coefficient': coefficients.values})
    ...
    if experiment == 'RestlessBandit':
        ...
        features_of_interest = ['No Of Parameters', 'Code',
                                'Size Of Dataset', 'Open Source', 'Use Of rlhf']
    # Plot a bar plot using seaborn
    barplot = sns.barplot(x='Coefficient', y='Feature',
                          data=coeff_df, xerr=cis, color='black', alpha=0.6)
    ...
    for i, p in enumerate(pvalues):
        ...
        x_pos = coeff_df['Coefficient'][i] + \
            cis[i] if coeff_df['Coefficient'][i] > 0 else coeff_df['Coefficient'][i] - cis[i]
    ...
    if regtype == 'behavior':
        ...
        plt.savefig(
            f'./plots/regressions/{experiment}/barplot_{score_name}.pdf')
    else:
    ...
    for idx, row in df.iterrows():
        ...
        for feature in llm_df.columns:
            if feature not in features_to_exclude:
                ...
                try:
                    df.loc[idx, feature] = llm_df.loc[llm_df['Engine']
                                                      == engine, feature].values[0]
...
def comparison_barplot(df,  experiment, score_name, engines, group_labels):
    ...
    dfcot = df.copy()
    dfcot = dfcot.rename(columns={f'{score_name}': 'score'})
    dfsb = df.copy()
    dfsb = dfsb.rename(columns={f'{score_name}': 'score'})
    ...
    agg_means, agg_cis = aggregated_results(
        [dfcot, dfsb], score_name, experiment)
    # ?
    # Loop across groups and plot
    for i, llm in enumerate(engines):
        mean_base, ci_base = get_means_and_cis(
            df[df['engine'] == llm[-1]], score_name)
        for idx, engine in enumerate(llm[:-1]):
            width = 0.9
            mean, ci = get_means_and_cis(
                df[df['engine'] == engine], score_name)
            mean = mean - mean_base
            ...
            if '_cot' in engine:
                ...
                grey_int = 0.7
            plt.bar(engine, mean, yerr=ci/2, width=width,
                    color=f'C0{i}', alpha=grey_int)
        ...
        legend_handles.append(mpatches.Patch(
            color=f'C0{i}', label=group_labels[i], alpha=0.5))
    ...
    if (experiment == 'ProbabilisticReasoning') & (score_name == 'performance_score1'):
        ...
        legend_handles[1] = mpatches.Patch(
            color=f'C01', label=f'text-bison', alpha=0.5)
        legend_handles[0] = mpatches.Patch(
            color=f'C00', label=f'GPT-4', alpha=0.5)
        legend_handles[2] = mpatches.Patch(
            color=f'C02', label=f'Claude-2', alpha=0.5)
        legend_handles[3] = mpatches.Patch(
            color=f'C03', label=f'Claude-1', alpha=0.5)
        ...
        legend_handles = [
            legend4] + [mpatches.Patch(color='white', label='')] + legend_handles[5:]

    #! Add two bars after some space for each experiment
    plt.bar(' ', 0, color='white')
    plt.bar('new', agg_means[0], yerr=agg_cis[0],
            color=f'tab:brown', alpha=0.3)
    plt.bar('new2', agg_means[1], yerr=agg_cis[1],
            color=f'tab:brown', alpha=0.7)
...
def radar(metrics_dict, agents_to_plot, interest, labels, squash=True):
    ...
    metrics = np.delete(
        metrics, [human_idx, random_idx] + idx_to_remove, axis=0)
    ...
    for i, (engine, _) in enumerate(metrics_dict.items()):
        ...
        plt.legend(bbox_to_anchor=(0, 1.02, 1, 0.2), loc='lower center',
                   ncol=3, frameon=False, columnspacing=0.25)
...
def umap_plot(metrics_dict, feature_of_interest, feature_of_interest_name,  interest='behaviour', standardize=True, name_all=True):
    ...
    # Create a color list
    colors = ['tab:red'if boolean == 'Yes' else 'tab:purple' if boolean ==
              'RLAIF' else 'C00' for boolean in feature_of_interest]
    colors = ['C01' if engine in ['random', 'human']
              else color for engine, color in zip(metrics_dict.keys(), colors)]
    ...
    for i, engine in enumerate(metrics_dict.keys()):
        if name_all == False:
            enginestoname = ['gpt-4', 'text-bison', 'human', 'random',
                             'llama-2-70-chat', 'llama-2-70', 'claude-2']  # Hardcoded for paper
            if engine in enginestoname:
                plt.annotate(
                    engine, (embedding[i, 0], embedding[i, 1]), fontsize='small')
    plt.gca().set_aspect('equal', 'datalim')

    # Create a legend
    red_patch = mpatches.Patch(
        color='tab:red', label=feature_of_interest_name, alpha=0.7)
    blue_patch = mpatches.Patch(
        color='C00', label='No ' + feature_of_interest_name, alpha=0.7)
    ...
    plt.legend(handles=[red_patch, blue_patch, purple_patch, orange_patch], frameon=False,
               bbox_to_anchor=(0, 1.02, 1, 0.2), loc='lower center', ncol=2, columnspacing=0.5)
    sns.despine()
    ...
    # Save the plot
    plt.savefig(
        f'./plots/umap/{interest}{feature_of_interest_name}{"_notstandardized" if not standardize else ""}`.pdf')
...
def calculate_distances(metrics_dict, feature_of_interest, embedding):
    ...
    # Calculate the distances to the "Human" point for the two groups
    rlhf_indices = [i for i, feature in enumerate(
        feature_of_interest) if feature == 'Yes']
    non_rlhf_indices = [i for i, feature in enumerate(
        feature_of_interest) if feature == 'No']
    rlhf_distances = np.linalg.norm(
        embedding[rlhf_indices] - human_coords, axis=1)
    non_rlhf_distances = np.linalg.norm(
        embedding[non_rlhf_indices] - human_coords, axis=1)
    ...
    print(
        f"Average distance to Humans for LLMs with Use of RLHF: {avg_rlhf_distance}")
    print(
        f"Average distance to Humans for LLMs without Use of RLHF: {avg_non_rlhf_distance}")
...
def merge_all_metrics_and_features(experiments, excluding_agents, llm_df):
    ...
    metrics = {engine: []
               for engine in including_agents if engine not in excluding_agents}
    metrics_cis = {engine: []
                   for engine in including_agents if engine not in excluding_agents}
            ...
            for engine in metrics.keys():
                ...
                if (experiment == 'BART') & (engine == 'human'):
                    ...
                    if score_name == 'behaviour_score1':
                        men = 32.2*(1+9.1/30) + 7.6 * \
                            (1+16.5/30) + 1.2*(1+22.2/30)
                    ...
                    if f'{score_name}_CI' in df.columns:
                        ci = df[df['engine'] ==
                                engine][f'{score_name}_CI'].values[0]
                    else:
                    ...
                    if np.isnan(mean_score):
                        print(
                            f"Engine {engine} has NaN values for {score_name} in experiment {experiment}")
                        # import ipdb
                        # ipdb.set_trace()
                        # Remove problematic engine from both dicts
