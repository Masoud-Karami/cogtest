...
def exclude_agents():
    ...
    '''
    excluding_agents = [
        'rational', 'meta-RL']  # These engines were exploratory for specific experiments and therefore do not have metrics for all experiments.
    #! Llama 1 variants to exclude because not included in the paper since they were replaced by llama 2
    excluding_agents += ['llama_65', 'llama_30', 'llama_13', 'llama_7', 'vicuna_13', 'vicuna_7',
                         'hf_vicuna-7b-v1.3', 'hf_vicuna-13b-v1.3', 'hf_vicuna-33b-v1.3', 'hf_koala-7B-HF', 'hf_koala-13B-HF']
...
def plot_data(dp, filename, metrics_names, behav=False, store_id='0'):
    ...
    if behav:
        colors = ['tab:green', 'tab:green', 'tab:orange', 'tab:orange', 'tab:blue', 'tab:red',
                  'tab:red', 'tab:brown', 'tab:cyan', 'tab:purple']  # Add more colors if needed
    else:
        colors = ['tab:green', 'tab:orange', 'tab:blue', 'tab:red',
                  'tab:brown', 'tab:purple']  # Add more colors if needed

    # Here hardcode names for paper and activated only if store_id is set to 0
    models_names = ["GPT-4", "text-davinci-003", "Claude-2",
                    "text-bison", "LLaMA-2-70", "LLaMA-2-70-chat"]
    for i, (ax, model) in enumerate(zip(axs, models)):
        dp_model = dp[dp['Model'] == model]
        sns.barplot(data=dp_model, x='Value', y='Task',  ax=ax,
                    alpha=0.6, hue='Task', palette=colors, dodge=False)
        # Loop to add error bar to each bar
        for j, metric in enumerate(metrics_names):
            ci = dp_model[dp_model['Task'] == metric]['CI'].values[0]
            ax.errorbar(dp_model[dp_model['Task'] == metric]
                        ['Value'], j, xerr=ci/2,  color='black', capsize=2)
        if store_id == '0':
...
def run(models=None, interest=None, store_id=None):
    ...
    parser = argparse.ArgumentParser(
        description='Compare models and optionally print scores.')
    parser.add_argument('--models', nargs='+', default=["gpt-4", "text-davinci-003", "claude-2", "text-bison@002", "llama-2-70",
                        "llama-2-70-chat"], help='List of models to compare. You need at least two models to compare and we recommend at most 7!')
    parser.add_argument('--print_scores', action='store_true',
                        help='If set, print the scores for each model.')
    parser.add_argument('--print_scores_for', nargs='+', default=[
                        "gpt-4", "human", "random"], help='List of models for which to print scores.')
    parser.add_argument('--store_id', type=str, default='',
                        help='If set, store the plot with this unique identifier.')
    parser.add_argument('--interest', choices=['behaviour', 'performance'], default='behaviour',
                        help='Specify whether you are interested in "behavior" or "performance" plots.')
    ...
    if interest == 'behaviour':
        experiments = {
            ...
            'SerialMemoryTask': ['behaviour_score1', 'behaviour_score2', 'behaviour_score3', 'behaviour_score4'],
        }
        metrics_names = ['Prior weighting', 'Likelihood weighting', 'Directed exploration', 'Random exploration',
                         'Meta-cognition', 'Learning rate', 'Optimism bias', 'Model-basedness',  'Temporal discounting', 'Risk taking', 'Initiation error rate', 'Intertrial forgetting', 'Primacy effect', 'Recency effect']
        ...
        experiments = {
            ...
            'SerialMemoryTask': ['performance_score1', 'performance_score2'],
        }
        metrics_names = ['Probabilistic Reasoning', 'Horizon Task',
                         'Restless Bandit', 'Instrumental Learning', 'Two Step Task', 'BART', 'Serial Memory Accuracy', 'Mean TTC']
   ...
   # Filter to selected models only (e.g., to exclude gpt-4 if not in args.models and only include Llama-2)
    ...
    metrics_cis = {k: v for k, v in metrics_cis.items() if k in models + ['random']} 
