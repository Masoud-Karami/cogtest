...
class Experiment:
    ...
    def add_arguments(self):
        self.parser.add_argument(
            '--engines', nargs='+', default=['random'], help='List of engines to use')
        self.parser.add_argument(
            '--max-tokens', type=int, default=2, help='Maximum number of tokens')
        self.parser.add_argument(
            '--temp', type=int, default=0, help='Temperature for LLM')
        self.parser.add_argument('--debug', action='store_true', default=False,
                                 help='Debug mode to print the output of the LLM at each step of the experiment')
        self.parser.add_argument('--num_runs', type=int, default=1,
                                 help='Number of independent runs of the full task.')
        self.parser.add_argument('--max_tokens', type=int, default=512,
                                 help='Max tokens generated by the model.')
    ...
    def run(self):
        ...
        print(
            f"Running experiment with engines: {engines}, num_runs: {args.num_runs} and max_tokens: {args.max_tokens}")
    ...
    def postprocess(self, text, text_input, default_text):
        ...
        if (self.step_back) or (self.cot):
            ...
            if (len(parts) == 1) or (len(parts[1].split('.')[0].split('\n')[0].split("'")[0].replace(' ', '')) == 0):
                ...
                print(
                    f"Answer not in right format, generating again by appending the format at the end...")
                ...
                new_text = self._generate(
                    text_input + reasoning_text, self.temperature, 5)
                # import ipdb; ipdb.set_trace()
                # print(new_text)
                ...
                if len(new_text.split('.')[0].split('\n')[0].split("'")[0].replace(' ', '')) == 0:
                    ...
                    new_text = self._generate(
                        text_input + reasoning_text + wrong_output + re_ask_question, self.temperature, 5)
                    if len(new_text.split('.')[0].split('\n')[0].split("'")[0].replace(' ', '')) == 0:
...
class RandomLLM(LLM):
    ...
    def random_fct(self):
        raise NotImplementedError(
            "Should set this random function depending on the task!")
...
class StoringScores:
    ...
    def __init__(self):
        self.columns = ['engine', 'run', 'performance_score1',
                        'performance_score1_name', 'behaviour_score1', 'behaviour_score1_name']
        self.parser = argparse.ArgumentParser(
            description="Get behavioral and performance scores of experiments and store them.")
        self.add_arguments()

    def add_arguments(self):
        self.parser.add_argument(
            '--engines', nargs='+', default=['all'], help='List of engines to use')
        self.parser.add_argument(
            '--no_perf_score', nargs='+',  default=[], help='Number of performance scores')
        self.parser.add_argument('--version_number', type=str, default='1',
                                 help='Version number of the experiment. This is used to store the results of the experiment with have some changes seperately')
